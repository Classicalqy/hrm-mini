...
from scripts.core_five_l_depth_long_rollout import (
    L_DEPTH_COLORS, absolute_checkpoint, advance_hrm_l, advance_rt_l, atomic_npz, initial_hrm_state,
    initial_rt_state,
)
...
def make_figures(output: Path, msd: list[dict[str,object]], execution: list[dict[str,object]], phase_rows: list[dict[str,object]]) -> None:
    import pandas as pd
    d=pd.DataFrame(msd); e=pd.DataFrame(execution)

    def style(condition: str, eval_l: int) -> tuple[str, str, str]:
        if condition == "RT":
            return "#777777", ":", "RT"
        if condition == "H2L1_h":
            return "#222222", "-", "H2L1-H, L=1"
        return L_DEPTH_COLORS[int(eval_l)], "-", f"H2L6-H, L={eval_l}"

    def draw_msd(axis: Any, clock: str, state: str, segment: int) -> None:
        selected = d[(d.clock == clock) & (d.segment == segment) &
                     ((d.state == state) | ((state == "h") & (d.state == "rt")))]
        for (condition, eval_l), group in selected.groupby(["condition", "eval_l"]):
            mean = group.groupby("lag")["mean"].mean().sort_index()
            color, line_style, label = style(str(condition), int(eval_l))
            axis.plot(mean.index, mean.values, color=color, linestyle=line_style, label=label)
        axis.set_xscale("log", base=2); axis.set_yscale("log", base=2); axis.grid(alpha=.25)
    ...
    for clock,state,title in (("h","h","H-clock H MSD"),("h","l","H-clock L MSD"),("l","l","L-clock L MSD")):
        ...
        for seg,axis in enumerate(axes,1):
            draw_msd(axis, clock, state, seg); axis.set_title(f"segment {seg}");axis.set_xlabel("H-update lag" if clock=="h" else "L-update lag")
    ...
    # A compact report figure that exposes the two clocks without requiring a
    # reader to match legends across three separate panels.
    figure, axes = plt.subplots(3, 4, figsize=(15, 8.2), sharey="row")
    specs = (("h", "h", "H clock: H state"), ("h", "l", "H clock: L state"), ("l", "l", "L clock: L state"))
    for row, (clock, state, label) in enumerate(specs):
        for segment, axis in enumerate(axes[row], 1):
            draw_msd(axis, clock, state, segment)
            axis.set_title(f"{label}, segment {segment}")
            axis.set_xlabel("H-update lag" if clock == "h" else "L-update lag")
            if segment == 1:
                axis.set_ylabel("per-coordinate MSD")
    axes[0, -1].legend(fontsize=7)
    figure.suptitle("Dual-clock full-state MSD")
    figure.tight_layout()
    figure.savefig(output / "dual_clock_msd.png", dpi=200); figure.savefig(output / "dual_clock_msd.pdf"); plt.close(figure)
    if not e.empty:
        ...
        for axis,metric in zip(axes,metrics):
            ...
            for (condition,l),g in q.groupby(["condition","eval_l"]):
                color, _line_style, label = style(str(condition), int(l)); y=g.groupby("segment")["mean"].mean(); axis.plot(y.index,y.values,marker="o",color=color,label=label)
    ...
    if not phase.empty:
        ...
        for seg,axis in enumerate(axes,1):
            ...
            for (condition,l),g in q.groupby(["condition","eval_l"]):
                color, _line_style, label = style(str(condition), int(l)); y=g.groupby("phase_center")["mean"].mean(); axis.plot(y.index,y.values,color=color,label=label)
